...
# Robust one-liner, no heredoc
entrypoint-info: venv
	@$(PYTHON) -c 'import importlib, importlib.metadata as md, sys, platform; print(f"Python: {platform.python_implementation()} {platform.python_version()} ({sys.executable})"); eps=[e for e in md.entry_points(group="console_scripts") if e.name=="talk"]; print("Entry point:", eps[0] if eps else "<none>"); mod_name=eps[0].value.split(":")[0] if eps else None; m=importlib.import_module(mod_name) if mod_name else None; print("Module file:", getattr(m, "__file__", None))'
