...
"""
Talk v3b - Planning-driven orchestration with VitalsAgent for health monitoring.
...
from special_agents.vitals_agent import VitalsAgent

# Configure logging
log = logging.getLogger("talk_v3b")


class TalkOrchestratorV3:
    """
    Talk v3b orchestrator with planning-driven architecture and health monitoring.
    ...
    - VitalsAgent monitors system health and implements rate limiting
    - All agents have single, focused responsibilities
    - Central control flow: work → plan → branch → vitals → work
    """
    ...
    def _create_plan_v3(self) -> List[Step]:
        """
        Create the v3b execution plan with central planning/branching and health monitoring.
        
        The flow is:
        1. Initial analysis (optional)
        2. Main loop: plan → branch → vitals → work → plan → branch → vitals → work...
        3. Work steps are selected dynamically by BranchingAgent
        4. VitalsAgent monitors health and can halt if critical
        """
        steps = []
        ...
        branch_step = Step(
            label="select_action",
            agent_key="branching",
            on_success="check_vitals",  # Goes to vitals check first
            on_fail="error_recovery"
        )
        steps.append(branch_step)
        
        # Step 4: Vitals check - monitors health and applies rate limiting
        vitals_step = Step(
            label="check_vitals",
            agent_key="vitals",
            on_success="dynamic",  # Will route to what BranchingAgent selected
            on_fail="error_recovery"
        )
        steps.append(vitals_step)
        ...
        # Create VitalsAgent to monitor system health (needs access to BranchingAgent)
        self.agents["vitals"] = VitalsAgent(
            monitored_step=branch_step,  # Monitor the branching step
            plan=steps,  # Complete plan for context
            vitals_step=vitals_step,  # Reference to its own step
            workspace_dir=str(self.working_dir),  # Pass workspace for file tracking
            name="SystemVitals"
        )
        # Give VitalsAgent access to BranchingAgent
        self.agents["vitals"].branching_agent = self.agents["branching"]
    ...
    def run(self) -> int:
        ...
        try:
            # Reset VitalsAgent stats for this run
            from special_agents.vitals_agent import VitalsAgent
            VitalsAgent.reset_stats(workspace_dir=str(self.working_dir))
