#!/usr/bin/env python3
# special_agents/file_agent_simple.py

"""
FileAgent - Simple version with the indentation fix.

This is the original FileAgent with just the line.strip() bug fixed.
It demonstrates that the indentation problem could have been solved
by simply preserving the original line content while only stripping
for command detection.

This version supports both CREATE_FILE format and unified diffs.
"""
...
class FileAgent(Agent):
    ...
    def _parse_operations(self, operations_text: str) -> List[Dict]:
        ...
        for line in operations_text.split('\n'):
            ...
            if stripped_line.startswith('CREATE_FILE:') or stripped_line.startswith('MODIFY_FILE:'):
                ...
                # Keep the original line with its indentation
